...
func handleSignals(session ssh.Session) {
  ...
  // Transform syscall signals to ssh signals
	go func(){
		for sig := range c {
			switch sig {
        case syscall.SIGABRT: _ = session.Signal(ssh.SIGABRT)
        case syscall.SIGALRM: _ = session.Signal(ssh.SIGALRM)
        case syscall.SIGFPE:  _ = session.Signal(ssh.SIGFPE)
        case syscall.SIGHUP:  _ = session.Signal(ssh.SIGHUP)
        case syscall.SIGILL:  _ = session.Signal(ssh.SIGILL)
        case syscall.SIGINT:  _ = session.Signal(ssh.SIGINT)
        case syscall.SIGKILL:  _ = session.Signal(ssh.SIGKILL)
        case syscall.SIGPIPE:  _ = session.Signal(ssh.SIGPIPE)
        case syscall.SIGQUIT:  _ = session.Signal(ssh.SIGQUIT)
        case syscall.SIGSEGV:  _ = session.Signal(ssh.SIGSEGV)
        case syscall.SIGTERM:  _ = session.Signal(ssh.SIGTERM)
        case syscall.SIGUSR1:  _ = session.Signal(ssh.SIGUSR1)
        case syscall.SIGUSR2:  _ = session.Signal(ssh.SIGUSR2)
      }
    }
	}()
}
